...
pushq %rdi       #move rdx/first input to current top of stack
subq $8, %rsp
...
power_loop:
    ...
    imulq %rdi , %rax     #multiply rdx (which is the base) with rax and store it in rax
    movq %rax, -8(%rbp)   #move rax/outcome of the multiplication to what was top of stack/ -8rbp
    decq %rsi             # decrement the exponent (second input, that is stored in rcx)
